...
def speculative_sampling(prefix: torch.Tensor, approx_model: torch.nn.Module, target_model: torch.nn.Module,
                         ...
                         random_seed: int = None, eos_token_id: int = 2) -> torch.Tensor:
    """
    ...
    while prefix.shape[1] < T:
        ...
        assert approx_model_cache._prob_history.shape[
                   -2] <= n + 1, f"approx_model prob list shape {approx_model_cache._prob_history.shape}, n {n}"
        ...
        if eos_token_id in prefix[0].tolist():
            break
    if verbose:
        print(
            f"generated tokens numbers {prefix.shape[-1] - seq_len}, accepted_count {accepted_count}, accept_rare {(accepted_count/(prefix.shape[-1] - seq_len))*100}, target_sample_count {target_sample_count}, resample_count {resample_count}")
    return prefix
